...
internals.parseArray = function (property, joiObj, name, definitionCollection, altDefinitionCollection, type, isAlt) {
    ...
    if (foundArray) {
        ...
        if (Hoek.reach(foundArray, '_settings.language.label')) {
            name = Hoek.reach(foundArray, '_settings.language.label');
        }
...
internals.parseAlternatives = function (property, joiObj, name, definitionCollection, altDefinitionCollection, type) {
    ...
    if (Hoek.reach(joiObj, '_inner.matches.0.schema')) {
        ...
        let childName = (internals.getName(child) || name);
        ...
        let altArray = joiObj._inner.matches.map((obj) => {
            let altName = (internals.getName(obj.schema) || name);
...
/**
 * get name from object properties
 *
 * @param  {Object} joiObj
 * @return {String || Null}
 */
internals.getName = function (joiObj) {
